  ...
  (
    ...
    return (
      <div data-slot="select-field" className="flex min-w-0 flex-col gap-1">
        ...
        >
          <span
            className={cn("flex items-center min-w-0 flex-1", sizeClasses.gap)}
          >
            {Icon && (
              <Icon
            ...
            <SelectContentContext.Provider value={contentCtx}>
              <SelectPrimitive.Viewport asChild>
                <Elevated
                  ...
                  className={cn(
                    `select-popup flex flex-col min-w-[min(var(--radix-select-trigger-width),var(--radix-select-content-available-width))] max-w-[var(--radix-select-content-available-width)] max-h-[min(320px,var(--radix-select-content-available-height))] overflow-hidden ${shape.container} select-none outline-none`,
                    className,
                  )}
                >
                  <ScrollArea
                  ...
                  >
                    ...
                    >
                      {/* Unmount overlays on close so reopening cannot reuse stale animation geometry. */}
                      {open && (
                        <AnimatePresence>
                          {checkedRect && (
                            <motion.div
