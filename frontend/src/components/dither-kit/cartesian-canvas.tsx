...
function startCartesianLoop({
  ...
  const paintStars = (s: ChartContextValue, revealCols: number) => {
  ...
  const easeSurfaces = (s: ChartContextValue) => {
  ...
  const updateEntrance = (s: ChartContextValue, now: number) => {
    if (bloomCtx) {
      const on =
  ...
  const updateHover = (s: ChartContextValue) => {
  ...
  const draw = (now: number) => {
    ...
    const prog = updateEntrance(s, now);
    ...
    const moving = easeSurfaces(s);
    const settling = updateHover(s);
    ...
    paintMarker(s, marker, revealCols);

    paintStars(s, revealCols);
  };
