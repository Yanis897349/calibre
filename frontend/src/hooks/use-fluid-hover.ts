...
export function pickNearest({
  ...
  const scaleX = layoutScale(containerRect.width, layoutSize.width);
  const scaleY = layoutScale(containerRect.height, layoutSize.height);
  ...
  for (let index = 0; index < rects.length; index++) {
    ...
    if (axis === "xy") {
      ...
      if (containsPoint(point, left, top, width, height)) {
...
export function useFluidHover<T extends HTMLElement>(
  ...
  const runMeasurement = useCallback(() => {
    ...
    itemsRef.current.forEach((element, index) => {
      // Boxless items invalidate the measurement pass; fixed-position items may still have valid sizes.
      const hasLayoutBox =
      ...
      // Use layout offsets to avoid counting animated transforms; include positioned ancestors.
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      // An item removed during its own click has already handled it; do not route the click again.
  ...
  // Keep overlays visible during resize measurements to prevent flicker.
  useEffect(() => {
    const container = containerRef.current;
...
function layoutScale(measured: number, layout: number) {
  return layout > 0 ? measured / layout : 1;
}
